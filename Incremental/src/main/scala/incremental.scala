...
df.write.format("bigquery").option("writeMethod","direct").mode("append").option("temporaryGcsBucket",bucket).save("searce-practice-data-analytics.hive_to_bigquery."+string_row)
