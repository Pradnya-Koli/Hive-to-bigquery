...
def  Alter_add_columns():
    with open('query_hive.csv') as file_obj:
        ...
        for row in reader_obj:
            print(row)
            text = str(row)
            format_query = text.replace(".", " ").replace("(", " ").replace(")", " ").replace("[", " ").replace("]",
            ...
            for i in phrase_to_list:
                ...
                if (i == 'ALTER'):
                    ...
                    for row in dataframe.index:
                        if (row == 3):
                            table_id = "searce-practice-data-analytics.hive_to_bigquery.{}".format(dataframe[0][row])
                            ...
                            for row in dataframe.index:
                                if (row == 6):
                                    while row != length_list:
                                        ...
                                        new_schema.append(bigquery.SchemaField(value, data_type))
        ...
        for row in reader_obj:
            ...
            for row in dataframe.index:
                if (row == 3):
                    ...
                    table = client.get_table(table_id)
                if(row == 5):
                    query_rename=("ALTER TABLE {} RENAME COLUMN {} TO {};").format(table_id,dataframe[0][row],dataframe[0][row+1])
        ...
        for row in reader_obj:
            ...
            for row in dataframe.index:
                if (row == 2):
                    ...
                    table = client.get_table(table_id)
                if (row == 5):
                    query=("""ALTER TABLE `{}` DROP COLUMN IF EXISTS {};""").format(table_id,dataframe[0][row])
                    print(query)
